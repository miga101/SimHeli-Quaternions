...
glmLinearTexture(GLMmodel* model)
{
  ...
  /* do the calculations */
  for(i = 1; i <= model->numvertices; i++) {
    y = model->vertices[3 * i + 0] * scalefactor*0.95;//c1
    z = model->vertices[3 * i + 2] * scalefactor*2.9;//c2
    model->texcoords[2 * i + 0] = (y + 1.0) / 2.0;//c0
    model->texcoords[2 * i + 1] = (z + 1.0) / 2.0;//c1
  }
